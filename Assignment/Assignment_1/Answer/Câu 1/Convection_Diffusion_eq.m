...
%                 x_cp(ii) = 1/3.0*x(ii-1)+2/3.0*x(ii);
%                 x_cp(ii) = 2/3.0*x(ii-1)+1/3.0*x(ii);
            end
        end
    end
    %---------- Dicrete solution -----------%
    u_dis = Dirichlet(x,x_cp,N,u0,uN,v);
...
hold off
figure
plot(log(ll),-log(norml2)+1.5,'-x red', log(ll), -log(normh1),'-x blue', log(ll),1.5*log(ll)-0.5, 'black', log(ll), 2*log(ll)+0.5,'green');
title('Error');
